...
class KalmanFilterFusionNode(KalmanFilterNode):
    ...
    def set_observation(self):
        # Use 1D array format to match observation model output
        self.z = np.array([self.normalized_pose[0], self.normalized_pose[1], self.normalized_pose[2], 
                          self.normalized_imu_theta, self.imu_w, self.imu_a_x, self.imu_a_y])
